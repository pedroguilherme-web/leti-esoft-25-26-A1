...
__AC3:__ The list must include, at a minimum, the Name, Phone Number, and E-mail for each employee.

__AC4:__ If no employees are registered for the selected function/role, the system must display an empty list or an informative message.
...
__Input Data (After system presents the list for selection) :__

Selected data: The Administrator selects a Function/Role (e.g., "Nurse" or "Receptionist").

__Output Data:__

The system presents the list of available roles (for selection).

The system presents the list of employees (Name, Phone, Email) corresponding to the selected role.
...
| Step 2  		     | Coordinating the request for the employee list     | GetEmployeeListController           | Controller: Receives the event from the UI  and manages the flow of the user story.                                                 |
| Step 3  		     | Dispatching the request based on the selected role | GetEmployeeListController           | Controller: Contains the alt logic to decide which container to call.                                                               |
| Step 4  		     | Knowing all instances of NurseUser                 | NurseContainer                      |                                                                                                                                     |
| Step 5  		     | Knowing all instances of ReceptionistUser          | ReceptionistContainer               |                                                                                                                                     |
| Step 6  		     | Creating the EmployeeList                          | GetEmployeeListController           | Creator: Creates EmployeeList because it has the data (from the containers) needed for their initialization, ensuring Low Coupling. |
